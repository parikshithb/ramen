...
func (v *VRGInstance) undoPVCFinalizersAndPVRetentionForVGR(vrNamespacedName types.NamespacedName,
	...
	if v.hasGlobalVGRLabel() {
		// Remove ramen finalizers and annotations from PVCs so they can be cleaned up
		// on the secondary. The shared VGR is only deleted when all VRGs using it are
		// being deleted.
		for idx := range pvcs {
		...
		if !v.isGlobalDeleteInConsensus() {
